...
dt = 0.1;
vm = 0.2;                   %attacker linear velocity
max_vt = 0.4;               %maximum target linear velocity
vd = 4;                     %defender linear velocity
e = 3;                      %safety distance between the attacker and target
K = 10;                     
N = 3;                     %proportional constant of Proportional Navigation
...
%%     'StageConstraints'     , {BoxSet( [-0.8; 0 ; -2],12:14,[0.8 ; 0.4 ; 2],12:14,14)},...
%% 'StageConstraints'     , {BoxSet( [-0.6; 0 ; -2],12:14,[0.6 ; 0.5 ; 2],12:14,14)},...
mpcOp = ICtMpcOp( ...
    'System'               , sys,...
    'HorizonLength'        , 2*dt,...
    'StageConstraints'     , {BoxSet( [-0.8; 0 ; -0.8],12:14,[0.8 ; 0.4 ; 0.8],12:14,14)},...
    'StageCost'            , @(t,x,u,varargin) (1-not(not(floor(e/(sqrt((x(6)-x(1))^2+(x(7)-x(2))^2))))))*(u(2)) ...+ u(3)^2)...
