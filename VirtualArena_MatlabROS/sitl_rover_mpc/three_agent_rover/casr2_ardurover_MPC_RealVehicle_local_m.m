classdef casr2_ardurover_MPC_RealVehicle_local_m < CtSystem
    ...
    methods
        ...
        function obj = casr2_ardurover_MPC_RealVehicle_local_m(N, ...                                 
                                                           K, ...
                                                           switchGuidanceLaw, ...
                                                           vm, ...
                                                           vd, ...
...
%                     ((((-1).^floor(ceil(t/obj.switchGuidanceLaw)))+1)/2)*(obj.N*((obj.vt*sin(target_angle.Data-x(9))-obj.vm*sin(x(10)-x(9)))/x(8)))+(1-((((-1).^floor(ceil(t/obj.switchGuidanceLaw)))+1)/2))*(((-obj.K*(x(10)-x(9)))/obj.vm))];  %Angular Velocity of Target
 ...
 %           qwerty = tic;
            ...
            %attacker_utm = receive(obj.attacker_utm_subscriber,10);
...
%            disp(['-attacker receive time-',num2str(qwerty1)]);
            ...
            qwerty2 = tic;
            %target_utm = receive(obj.target_utm_subscriber,10);
...
%             disp(['-target receive time-',num2str(qwerty3)]);
          ...
          %  defender_utm = receive(obj.defender_utm_subscriber,10);
...
%             %% Convert to UTM
%             
%             [attacker_utmX, attacker_utmY] = deg2utm(attacker_utmX, attacker_utmY);
